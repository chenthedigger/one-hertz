...
## Current state (2026-08-27) — 🚢 SHIPPED PUBLIC

- **https://github.com/chenthedigger/one-hertz PUBLIC** · live https://one-hertz.ubonranto.workers.dev (homepage set) · P6 gate tune list 9/9 applied + deployed v3108f93f (deep-link residency bug fixed, ?scroll lands at money moment) · founder decisions executed: D1 ubonranto accepted (custom domain = later option), D2+D3 history rewrite done — 945→215MB, Apple raw textures + 250 superseded evidence files purged from all history (keep-list verified zero collisions; full pre-slim history recoverable from ~/engineer/one-hertz-preslim.bundle) · force-pushed 353b891, 36 commits narrative intact · carried post-ship: real mid-tier device perf round, custom domain option.

## Prior state (P6 pre-flip)
